...
// Reminds teachers who haven't marked ANY attendance today for ANY of
// their assigned students. Runs daily — adjust the schedule/timezone to
// suit your class hours (this default assumes evening classes and checks
// at 19:00 local time).
exports.attendanceMarkingReminder = onSchedule(
  { schedule: 'every day 19:00', timeZone: 'Asia/Riyadh' },
  async () => {
    ...
    const markedTodayStudentIds = attendanceSnap.docs
      .map((d) => d.data())
    ...
    teachers.forEach((teacher) => {
      const myStudentIds = students
        ...
        .map((s) => s.id);
      if (!myStudentIds.length) return; // no students assigned, nothing to remind about

      const anyMarked = myStudentIds.some((id) => markedTodayStudentIds.includes(id));
      if (anyMarked) return;

      const body = "You haven't marked today's attendance yet for your students.";
      const id = generateId('NF');
